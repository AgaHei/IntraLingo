    ...
    with col1:
        ...
        if uploaded_file is not None:
            try:
                ...
                # Check file size
                if file_size_mb > 10:
                    st.error(f"❌ File too large: {file_size_mb:.1f}MB. Maximum allowed: 10MB")
                    uploaded_file = None
                else:
                    # Simple validation - just check if we can read bytes
                    try:
                        ...
                        if len(file_bytes) == 0:
                            ...
                            st.success(f"✅ File validation passed - {len(file_bytes)} bytes read")
                ...
                st.error(f"❌ Unexpected file handling error: {str(e)}")
                st.exception(e)
                uploaded_file = None
